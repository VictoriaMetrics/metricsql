...
func TestPrettier(t *testing.T) {
	...
	check(`{}[5m:3s]`, `{}[5m:3s]`)
	check(`{}[ 5m : 3s ]`, `{}[5m:3s]`)
	check(`{} offset 5m`, `{} offset 5m`)
	check(`{} offset -5m`, `{} offset -5m`)
	check(`{} offset 5M`, `{} offset 5M`)
	check(`{}[5m] offset 10y`, `{}[5m] offset 10y`)
	check(`{}[5.3m:3.4s] offset 10y`, `{}[5.3m:3.4s] offset 10y`)
	check(`{}[:3.4s] offset 10y`, `{}[:3.4s] offset 10y`)
	check(`{}[:3.4s] offset -10y`, `{}[:3.4s] offset -10y`)
	check(`{Foo="bAR"}`, `{Foo="bAR"}`)
	check(`{foo="bar"}`, `{foo="bar"}`)
	check(`{foo="bar"}[5m]`, `{foo="bar"}[5m]`)
	check(`{foo="bar"}[5m:]`, `{foo="bar"}[5m:]`)
	check(`{foo="bar"}[5m:3s]`, `{foo="bar"}[5m:3s]`)
	check(`{foo="bar"} offset 13.4ms`, `{foo="bar"} offset 13.4ms`)
	check(`{foo="bar"}[5w4h-3.4m13.4ms]`, `{foo="bar"}[5w4h-3.4m13.4ms]`)
	check(`foo{bar="baz"}[5m:4s] @ (end() - 3.5m) offset 2.4h`, `foo{bar="baz"}[5m:4s] offset 2.4h @ (end() - 3.5m)`)
	check(`{foo="bar"} offset 10y`, `{foo="bar"} offset 10y`)
	check(`{foo="bar"} offset -10y`, `{foo="bar"} offset -10y`)
	check(`{foo="bar"}[5m] offset 10y`, `{foo="bar"}[5m] offset 10y`)
	check(`{foo="bar"}[5m:3s] offset 10y`, `{foo="bar"}[5m:3s] offset 10y`)
	check(`{foo="bar"}[5m] oFFSEt 10y`, `{foo="bar"}[5m] offset 10y`)
	check(`METRIC`, `METRIC`)
	check(`metric`, `metric`)
	check(`m_e:tri44:_c123`, `m_e:tri44:_c123`)
	check("-metric", "0 - metric")
	check(`metric offset 10h`, `metric offset 10h`)
	check(`metric[5m]`, `metric[5m]`)
	check(`metric[5m:3s]`, `metric[5m:3s]`)
	check("metric[5m] offset 10h", "metric[5m] offset 10h")
	check("metric[5m:3s] offset 10h", "metric[5m:3s] offset 10h")
	check("metric[5i:3i] offset 10i", "metric[5i:3i] offset 10i")
	check(`metric{foo="bar"}`, `metric{foo="bar"}`)
	check(`metric{foo="bar"} offset 10h`, `metric{foo="bar"} offset 10h`)
	check(`metric{foo!="bar"}[2d]`, `metric{foo!="bar"}[2d]`)
	check(`metric{foo="bar"}[2d] offset 10h`, `metric{foo="bar"}[2d] offset 10h`)
	check(`metric{foo="bar", b="sdfsdf"}[2d:3h] offset 10h`, `metric{foo="bar", b="sdfsdf"}[2d:3h] offset 10h`)
	check(`metric{foo="bar", b="sdfsdf"}[2d:3h] offset 10`, `metric{foo="bar", b="sdfsdf"}[2d:3h] offset 10`)
	check(`metric{foo="bar", b="sdfsdf"}[2d:3] offset 10h`, `metric{foo="bar", b="sdfsdf"}[2d:3] offset 10h`)
	// possibly bug
	check(`metric{foo="bar", b="sdfsdf"}[2:3h] offset 10h`, `metric{foo="bar", b="sdfsdf"}[2:3h] offset 10h`)
	check(`metric{foo="bar", b="sdfsdf"}[2.34:5.6] offset 3600.5`, `metric{foo="bar", b="sdfsdf"}[2.34:5.6] offset 3600.5`)
	check(`metric{foo="bar", b="sdfsdf"}[234:56] offset -3600`, `metric{foo="bar", b="sdfsdf"}[234:56] offset -3600`)
	check(`  metric  {  foo  = "bar"  }  [  2d ]   offset   10h  `, `metric{foo="bar"}[2d] offset 10h`)
	// // @ modifier
	// // See https://prometheus.io/docs/prometheus/latest/querying/basics/#modifier
	check(`foo @ 123.45`, `foo @ 123.45`)
	check(`foo\@ @ 123.45`, `foo\@ @ 123.45`)
	check(`{foo=~"bar"} @ end()`, `{foo=~"bar"} @ end()`)
	check(`foo{bar="baz"} @ start()`, `foo{bar="baz"} @ start()`)
	check(`foo{bar="baz"}[5m] @ 12345`, `foo{bar="baz"}[5m] @ 12345`)
	...
	// 	check(`(-histogram_quantile(0.9, -rate(foo[1m] @ start())))`, `  0
	// -
	//   histogram_quantile (
	//     0.9,
	//       0
...
func TestPrettierShot(t *testing.T) {
	...
	check(`foo{bar="baz"} @ start()`, `foo{bar="baz"} @ start()`)
	check(`foo{bar="baz"}[5m] @ 12345`, `foo{bar="baz"}[5m] @ 12345`)
	check(`foo{bar="baz"}[5m:4s] offset 5m @ (end() - 3.5m)`, `foo{bar="baz"}[5m:4s] offset 5m @ (end() - 3.5m)`)
	check(`foo{bar="baz"}[5m:4s] @ (end() - 3.5m) offset 2.4h`, `foo{bar="baz"}[5m:4s] offset 2.4h @ (end() - 3.5m)`)
	check(`foo @ start() + (bar offset 3m @ end()) / baz OFFSET -5m`, `  (
    foo @ start()
  )
+
  (
    bar offset 3m @ end() / baz offset -5m
  )`)
	// same(`sum(foo) @ start() + rate(bar @ (end() - 5m))`)
	// another(`time() @ (start())`, `time() @ start()`)
	// another(`time() @ (start()+(1+1))`, `time() @ (start() + 2)`)
...
// func TestPrettier2(t *testing.T) {
// 	tests := []struct {
// 		input string
// 		want  string
// 	}{
...
// +
//   forth`},
// 		// {input: "(node_timex_offset_seconds > 0.05 and deriv(node_timex_offset_seconds[5m]) >= 0) or (node_timex_offset_seconds <=0.05 and deriv(node_timex_offset_seconds[5m]) <= 0)", want: ""},
// 		{input: `(-histogram_quantile(0.9, -rate(foo[1m] @ start())))`, want: `  0
// -
//   histogram_quantile (
//     0.9,
//       0
